...
const maxkb = reactive({ baseUrl: '', apiKey: '', timeoutSeconds: 30 })
...
function loadConfigsToForm() {
  ...
  maxkb.timeoutSeconds = Number.isFinite(Number(store.maxKBConfig.timeoutSeconds))
    ? Math.min(300, Math.max(30, Math.round(Number(store.maxKBConfig.timeoutSeconds))))
    : 30
...
.field-label-with-help { display: inline-flex; align-items: center; gap: 8px; }
.field-label-with-help :deep(.el-tooltip__trigger) { display: inline-flex; align-items: center; color: var(--muted); cursor: help; }
.timeout-field { display: flex; align-items: center; min-width: 0; }
.timeout-field :deep(.el-input-number) { width: 324px; }
...
@media (max-width: 640px) {
  ...
  .timeout-field { align-items: stretch; }
  .timeout-field :deep(.el-input-number) { width: 100%; }
