...
import { CalendarClock, Database, FolderOpen, MoreHorizontal, Pencil, Play, Trash2 } from 'lucide-vue-next'
import { ElMessageBox, ElSwitch, ElTag, ElTooltip } from 'element-plus'
import type { FolderDTO } from '../types'

const props = defineProps<{ folder: FolderDTO; busy?: boolean; processing?: boolean; workspaceNameLoading?: boolean }>()
const emit = defineEmits<{ (e: 'sync', folderId: string): void; (e: 'edit', folderId: string): void; (e: 'delete', folderId: string): void; (e: 'toggle-enabled', folderId: string, currentEnabled: boolean): void }>()
    ...
    <div class="task-destination">
      <div>
        ...
        <el-tooltip v-if="folder.workspaceId" :content="`ID：${folder.workspaceId}`" placement="top" :show-after="150">
          <strong class="destination-value">{{ folder.workspaceName || (workspaceNameLoading ? '名称加载中' : '名称未找到') }}</strong>
        </el-tooltip>
