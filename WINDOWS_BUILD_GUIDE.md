...
- 所有用户可以使用；
- 复制文件和设置 ACL 时需要管理员权限；
- 开始安装时 Windows 会显示一次 UAC，后续启动应用不显示 UAC。
...
├─ config\          配置目录
├─ logs\            日志目录
└─ data\            SQLite、快照、临时文件和备份
```

安装器会根据安装范围设置 `config/logs/data` ACL：仅当前用户模式授予当前账户修改权限；所有用户模式向 Windows 内置 Users 组授予继承的修改权限。程序不会把运行数据写入 `app`。

| 场景 | 安装时 UAC | 启动 MaxKB 时 UAC | 数据 |
| --- | ---: | ---: | --- |
| 仅当前用户 | 不需要 | 不需要 | 当前用户获得 `config/logs/data` 写权限 |
| 所有用户 | 需要 | 不需要 | 本机普通用户获得 `config/logs/data` 写权限 |

安装器交互界面始终以普通权限运行。所有用户模式会启动一个静默提权子进程执行文件复制、ACL 和 HKLM 注册；原安装器等待其完成后显示完成页，所以勾选“立即启动”时应用仍以普通权限运行。开始菜单和桌面快捷方式也不会请求管理员权限。

Windows 应用从 `<安装目录>\app` 的可执行文件位置解析安装根目录。首次使用新布局时，会把旧 `%LOCALAPPDATA%\MaxKB\MaxKB 本地文件同步工具` 或 `~\.maxkb-sync` 中的数据迁移到新目录。升级或卸载只删除 `app` 和安装注册信息，默认保留 `config/logs/data`。
